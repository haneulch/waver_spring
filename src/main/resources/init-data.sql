...
INSERT INTO waver.badge (achieve_count, achieve_yn, badge_type_id, select_yn, user_id)
SELECT 0, 'Y', 1, 'Y', 9999990
FROM DUAL
WHERE NOT EXISTS (
    SELECT 1 FROM waver.badge WHERE user_id = 9999990
);
...
FROM DUAL
WHERE NOT EXISTS (
    SELECT 1 FROM waver.badge WHERE user_id = 9999991
);
...
FROM DUAL
WHERE NOT EXISTS (
    SELECT 1 FROM waver.badge WHERE user_id = 9999992
);

-- Default Badge Type
INSERT IGNORE INTO waver.badge_type
