...
public record UserElement(
    ...
    public static UserElement of(User user) {
        Badge selected = user.getBadges().stream().filter(item -> item.getSelectYn() == YesNo.Y).findFirst()
            .orElse(null);
        if (selected == null) {
        ...
        return new UserElement(user.getId(), user.getName(), user.getImgUrl(), selected.getBadgeType().getTitle(),
            badgeImgUrl);
    }
}
